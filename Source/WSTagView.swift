...
open class WSTagView: UIView {
    ...
    public init(tag: WSTag) {
        ...
        textLabel.frame = CGRect(x: hPadding, y: vPadding, width: 0, height: 0)
        textLabel.font = font
    ...
    open override var intrinsicContentSize: CGSize {
        ...
        let hPadding = horizontalPadding ?? Constants.TagViewXPadding
        let vPadding = verticalPadding ?? Constants.TagViewYPadding
        return CGSize(width: labelIntrinsicSize.width + 2 * hPadding,
                      height: labelIntrinsicSize.height + 2 * vPadding)
    }

    open override func sizeThatFits(_ size: CGSize) -> CGSize {
        let hPadding = horizontalPadding ?? Constants.TagViewXPadding
        let vPadding = verticalPadding ?? Constants.TagViewYPadding
        let fittingSize = CGSize(width: size.width - 2.0 * hPadding,
                                 height: size.height - 2.0 * vPadding)
        let labelSize = textLabel.sizeThatFits(fittingSize)
        return CGSize(width: labelSize.width + 2.0 * hPadding,
                      height: labelSize.height + 2.0 * vPadding)
    }
